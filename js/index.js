...
function show() {
    fetch('http://localhost:3000/tasks')
        ...
        .then(function (tasks) {
            console.log(tasks);
            var html = '<thead><tr><th>Имя</th><th>Описание</th><th>Действия</th></tr></thead><tbody>';
            for (var i = 0, len = tasks.length; i < len; i++) {
                var task = tasks[i];
                var deleteButton = '<button class=\'btn btn-danger\' onclick="deleteTask(\'' + task._id + '\')">Удалить</button>';
                var editButton = '<button class=\'btn btn-primary\' onClick="showEditWindow(\'' + task._id + '\',\'' + task.name + '\',\'' + task.description + '\')">Редактировать</button>'
